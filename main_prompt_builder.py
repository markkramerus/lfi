...
def build_main_prompt(scenario, agent_config):
    ...
    parts.append('When your goal has been achieved, start wrapping up the conversation by asking if there is anything else the other party needs, or if they have any more questions. A tool may also signal you to start bringing the conversation to a conclusion by including "START WRAPPING UP THE CONVERSATION" in its output. When there are no more questions and the loose ends are tied up, end the conversation by appending "END OF CONVERSATION" to your final message. This will terminate the conversation. Use "END OF CONVERSATION" only when both parties have no more questions or activities. Be polite, and do not cut off abruptly, but at the same time, do not drag the conversation out for no reason.')
    parts.append('</ENDING THE CONVERSATION>')
    return '\n'.join(parts)
